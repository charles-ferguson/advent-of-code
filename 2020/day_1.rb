...
def find_summand_for(sum = 2020, numbers = NUMBERS)
  possible_summands = numbers.sort

  while possible_summands.count > 1
    lowest = possible_summands.first
    highest = possible_summands.last

    case lowest + highest
    when sum
      return [lowest, highest]
    when proc { |n| n < sum }
      possible_summands.shift
    when proc { |n| n > sum }
      possible_summands.pop
    end
  end
